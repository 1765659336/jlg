<template>
	<div>
		<el-table :data="errorTableData.sort((a, b) => b.timestamp - a.timestamp)" style="width: 100%">
			...
			<el-table-column prop="behavior" label="行为记录">
				<template #default="{ row }">
					<el-button type="primary" link @click="openBehaviorTable(row.behavior)"> 查看 </el-button>
				</template>
			</el-table-column>
			<el-table-column prop="rrwebUrl" label="回放">
				<template #default="{ row }">
					<el-button type="primary" link @click="openRrwebUrl(row.rrwebUrl)"> 查看 </el-button>
				</template>
			</el-table-column>
		</el-table>

		<el-dialog title="回放内容" v-model="dialogVisible" fullscreen destroy-on-close>
			<div ref="player" class="player-container"></div>
		</el-dialog>

		<el-dialog title="行为记录" v-model="dialogVisibleTable" fullscreen destroy-on-close @close="behaviorTableData = []">
			<el-table :data="behaviorTableData.sort((a, b) => b.timestamp - a.timestamp)" style="width: 100%">
				<el-table-column prop="timestamp" label="Timestamp" width="180">
					<template #default="{ row }">
						{{ formatTimestamp(row.timestamp) }}
					</template>
